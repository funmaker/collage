...
export default class EditorPage extends React.Component {
  ...
  async onImageClip(image, rows, columns) {
    ...
    this.setState(state => ({
      clipImage: null,
      images: [...state.images, result.image],
    }));
  }
  ...
  async onMoveImage(id, posx, posy) {
    ...
    this.setState(state => ({
      images: [...state.images.filter(img => img.id !== id), result.image],
    }));
  }
  ...
  async submitSettings(e) {
    ...
    const data = new FormIterator(e.target).serialize();
    ...
    try {
      ...
      if(data.resetImages) {
        ...
        this.setState(state => ({
          ...
          images: state.images.map(image => result.movedImages.includes(image.id) ? {
            ...image,
            posx: null,
            posy: null,
          } : image),
